...
*/
  ...
  SUBTYPE gt_all_constraints_rec      IS cort_all_constraints%ROWTYPE;
  ...
  AS
  BEGIN
    dbms_session.set_context('CORT_CONTEXT',in_name,in_value);
  END set_context;
  ...
  PRAGMA autonomous_transaction;
    l_log_id     NUMBER; 
  BEGIN
    IF in_ddl IS NOT NULL AND LENGTH(in_ddl) > 0 THEN
      IF in_echo OR g_params.debug THEN
        echo(in_ddl||';');
      ELSIF NOT in_echo THEN 
        g_ddl_arr(g_ddl_arr.COUNT+1) := in_ddl;
      END IF;  
      l_log_id := cort_log_pkg.log(
        in_text      => 'EXECUTE_IMMEDIATE',
        in_params    => in_ddl
      );
      $IF dbms_db_version.version >= 11 AND dbms_db_version.release >=1 $THEN
        EXECUTE IMMEDIATE in_ddl;
      $ELSE
        execute_immediate_10g(in_ddl);
      $END
  ...
  FUNCTION run_checks(
    ...
    in_check_stmt_arr   IN arrays.gt_xlstr_arr
  )
  RETURN NUMBER
  AS
    l_sql              VARCHAR2(32767);
    l_handle           NUMBER;
    l_check_result_arr arrays.gt_num_arr;
    l_cnt              NUMBER;
    l_result           NUMBER;
  BEGIN
    l_result := cort_comp_pkg.gc_result_alter;
    IF in_check_stmt_arr.COUNT = 0 THEN
      RETURN l_result;
    END IF;

    l_sql := 'SELECT ';
    FOR i IN 1..in_check_stmt_arr.COUNT LOOP
      l_sql := l_sql || in_check_stmt_arr(i)||',';
      l_check_result_arr(i) := NULL;
    END LOOP;
    l_sql := TRIM(',' FROM l_sql);
    l_sql := l_sql || ' FROM "'||in_source_table_rec.rename_rec.current_name||'"';

    debug('check sql: '||l_sql);

    l_handle := dbms_sql.open_cursor;
    BEGIN
      dbms_sql.parse(l_handle, l_sql, dbms_sql.native);
      FOR i IN 1..in_check_stmt_arr.COUNT LOOP
        dbms_sql.define_column(l_handle, i, l_check_result_arr(i));
      END LOOP;
      l_cnt := dbms_sql.execute_and_fetch(l_handle);
      FOR i IN 1..in_check_stmt_arr.COUNT LOOP
        dbms_sql.column_value(l_handle, i, l_check_result_arr(i));
      END LOOP;
    EXCEPTION
      WHEN OTHERS THEN
        g_error_stack := g_error_stack||dbms_utility.format_error_backtrace||CHR(10);
    ...
    FOR i IN 1..l_check_result_arr.COUNT LOOP
      l_result := GREATEST(l_result, l_check_result_arr(i));
    END LOOP;
  ...
  PROCEDURE apply_changes(
    ...
    in_raise_error         IN BOOLEAN DEFAULT TRUE,
  ...
  BEGIN
    ...
    WHILE l_indx IS NOT NULL LOOP
      IF LENGTH(in_frwd_alter_stmt_arr(l_indx)) > 0 THEN
        IF in_test THEN
          output(in_frwd_alter_stmt_arr(l_indx)||';');
        ELSE  
          BEGIN
            execute_immediate(in_frwd_alter_stmt_arr(l_indx), in_echo);
          EXCEPTION
            ...
            WHEN OTHERS THEN
              ...
              WHILE l_indx IS NOT NULL LOOP
                execute_immediate(in_rlbk_alter_stmt_arr(l_indx), in_echo);
                l_indx := in_rlbk_alter_stmt_arr.PRIOR(l_indx);
              END LOOP;
  ...
  PROCEDURE apply_changes_no_rollback(
    in_stmt_arr    IN arrays.gt_clob_arr, -- forward alter statements
    in_raise_error IN BOOLEAN DEFAULT TRUE,
    in_test        IN BOOLEAN DEFAULT FALSE, 
    in_echo        IN BOOLEAN DEFAULT TRUE
  )
  ...
  BEGIN
    apply_changes(
      in_frwd_alter_stmt_arr => in_stmt_arr,
      in_rlbk_alter_stmt_arr => l_rlbk_alter_stmt_arr, 
      in_raise_error         => in_raise_error,
      in_test                => in_test,
      in_echo                => in_echo
    );
  END apply_changes_no_rollback;
  ...
  AS
  BEGIN
    RETURN 'DROP TABLE "'||in_owner||'"."'||in_table_name||'" CASCADE CONSTRAINTS';
  END get_drop_table_ddl;

  -- Private - Drops object if it exists
  ...
  BEGIN
    ...
    THEN
      IF in_object_type = 'TABLE' THEN
        l_cascade_lause := ' CASCADE CONSTRAINTS';
      END IF;
      set_context('DROP_'||in_object_type,in_object_name);
      l_sql := 'DROP '||in_object_type||' "'||in_owner||'"."'||in_object_name||'"'||l_cascade_lause;
      execute_immediate(l_sql, in_echo);
      set_context('DROP_'||in_object_type,NULL);
    END IF;
  ...
  AS
    l_sql VARCHAR2(32767);
  BEGIN
    l_sql := 'PURGE '||in_object_type||' "'||in_owner||'"."'||in_object_name||'"';
    execute_immediate(l_sql);
  END exec_purge_object;
  ...
  BEGIN
    RETURN TO_CHAR((TO_NUMBER(TO_CHAR(l_sysdate, 'J')) - TO_NUMBER(TO_CHAR(date'2001-01-01', 'J')))*86400 + TO_NUMBER(TO_CHAR(l_sysdate, 'SSSSS')),'fm0XXXXXXXX');  
  END get_time_str;
  ...
  FUNCTION get_object_temp_name(
    ...
    in_owner       IN VARCHAR2,
    in_prefix      IN VARCHAR2 DEFAULT gc_temp_prefix
  )
  RETURN VARCHAR2
  AS
    l_temp_name VARCHAR2(24);
    l_max_name  VARCHAR2(30);
    l_cnt       PLS_INTEGER := 1;
  BEGIN
  ...
  BEGIN
    l_object_type := CASE in_object_type 
                       ...
                       WHEN 'LOB'     THEN 'LOB'
                     END; 
    l_temp_name := SUBSTR(in_prefix||l_object_type||get_time_str||TO_CHAR(SYS_CONTEXT('USERENV','SID'),'fm0XXXXX'),1, 24);
    WHILE check_object_exist(in_owner, l_temp_name) LOOP
      l_temp_name := get_next_name(l_temp_name);
    END LOOP;
    RETURN l_temp_name;
  ...
  AS
    ...
    TYPE t_lob_templates_arr          IS TABLE OF cort_all_lob_templates%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  BEGIN
    ...
    out_table_rec.physical_attr_rec.storage.buffer_pool := l_all_tables_rec.buffer_pool;
    ...
    IF out_table_rec.iot_type IS NOT NULL THEN
      ...
      out_table_rec.physical_attr_rec.storage.buffer_pool := l_indexes_rec.buffer_pool;
      ...
      BEGIN
        ...
        out_table_rec.overflow_physical_attr_rec.storage.buffer_pool := l_all_tables_rec.buffer_pool;
    ...
    IF out_table_rec.partitioned = 'YES' AND out_table_rec.iot_type IS NULL THEN
      ...
      out_table_rec.physical_attr_rec.storage.initial_extent := l_part_tables_rec.def_initial_extent;
      out_table_rec.physical_attr_rec.storage.next_extent := l_part_tables_rec.def_next_extent;
      out_table_rec.physical_attr_rec.storage.min_extents := l_part_tables_rec.def_min_extents;
      out_table_rec.physical_attr_rec.storage.max_extents := l_part_tables_rec.def_max_extents;
      out_table_rec.physical_attr_rec.storage.pct_increase := l_part_tables_rec.def_pct_increase;
      ...
      out_table_rec.physical_attr_rec.storage.buffer_pool := l_part_tables_rec.def_buffer_pool;
    ...
    IF out_table_rec.partitioned = 'YES' AND out_table_rec.iot_type IS NOT NULL THEN
      ...
      out_table_rec.physical_attr_rec.storage.initial_extent := l_part_indexes_rec.def_initial_extent;
      out_table_rec.physical_attr_rec.storage.next_extent := l_part_indexes_rec.def_next_extent;
      out_table_rec.physical_attr_rec.storage.min_extents := l_part_indexes_rec.def_min_extents;
      out_table_rec.physical_attr_rec.storage.max_extents := l_part_indexes_rec.def_max_extents;
      out_table_rec.physical_attr_rec.storage.pct_increase := l_part_indexes_rec.def_pct_increase;
      ...
      out_table_rec.physical_attr_rec.storage.buffer_pool := l_part_indexes_rec.def_buffer_pool;
...
--    out_table_rec.table_name := out_table_rec.rename_rec.object_name;
    ...
    EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM "'||in_owner||'"."'||in_table_name||'" WHERE ROWNUM <= 1' INTO l_cnt;
  ...
  AS
    ...
    l_virt_col_expr            CLOB; -- virtual record's expressions
  BEGIN
    SELECT *
      BULK COLLECT
      INTO l_tab_cols_arr
    ...
    FOR i IN 1..l_tab_cols_arr.COUNT LOOP
      l_hash_value := cort_comp_pkg.get_hash_value(l_tab_cols_arr(i).qualified_col_name);
      io_table_rec.column_indx_arr(l_tab_cols_arr(i).column_name) := i;
      io_table_rec.column_hash_indx_arr(l_hash_value) := i;
      ...
      io_table_rec.column_arr(i).data_default := TRIM(l_tab_cols_arr(i).data_default);
      ...
      io_table_rec.column_arr(i).qualified_col_hash := l_hash_value;
      $IF dbms_db_version.version >= 11 AND dbms_db_version.release >=1 $THEN
      ...
      IF io_table_rec.column_arr(i).virtual_column = 'YES' THEN 
        l_virt_col_expr := l_virt_col_expr||' '||io_table_rec.column_arr(i).data_default;
      END IF;  
      $END
      ...
      IF io_table_rec.column_arr(i).data_type_owner IS NULL THEN
        ...
        END CASE;   
      END IF;
      io_table_rec.column_arr(i).temp_column_name := gc_temp_prefix||'COLUMN_'||TO_CHAR(i,'fm0XXX');
    END LOOP;
/*    
    -- start from the end and mark all last virtual non partition key columns as droppable
    FOR i IN REVERSE 1..io_table_rec.column_arr.COUNT LOOP
      IF io_table_rec.column_arr(i).virtual_column = 'YES' AND 
         io_table_rec.column_arr(i).hidden_column = 'NO' AND 
         NOT io_table_rec.column_arr(i).partition_key
      THEN    
        io_table_rec.column_arr(i).droppable := TRUE;
      ELSE   
        -- all other columns are nor droppable
        EXIT;
      END IF;  
    END LOOP;
*/
    ...
    END LOOP;
    $END
    -- check dependencies on virtual columns
    $IF dbms_db_version.version >= 11 AND dbms_db_version.release >=1 $THEN
      FOR i IN 1..io_table_rec.column_arr.COUNT LOOP
        IF io_table_rec.column_arr(i).virtual_column = 'NO' THEN
          io_table_rec.column_arr(i).virtual_col_dependency := REGEXP_INSTR(l_virt_col_expr, '[^/.]"'||io_table_rec.column_arr(i).column_name||'"[^/.]') > 0;
        ELSE
          io_table_rec.column_arr(i).virtual_col_dependency := FALSE;
        END IF;
      END LOOP;
    $END
  ...
  BEGIN
    ...
    ELSE
      l_hash_value := cort_comp_pkg.get_hash_value(in_column_name);
      IF in_table_rec.column_hash_indx_arr.EXISTS(l_hash_value) THEN
        RETURN in_table_rec.column_hash_indx_arr(l_hash_value);
      ELSE
        RETURN NULL;
      END IF;
    END IF;
  ...
  BEGIN
    ...
    io_index_rec.physical_attr_rec.storage.buffer_pool := in_all_indexes_rec.buffer_pool;
  ...
  PROCEDURE assign_part_index_rec(
    io_index_rec         IN OUT NOCOPY gt_index_rec,
    in_all_indexes_rec   IN gt_all_part_indexes_rec
  )
  AS
    l_object_id INTEGER;
  BEGIN
    io_index_rec.tablespace_name   := in_all_indexes_rec.def_tablespace_name;
    io_index_rec.physical_attr_rec.pct_free  := in_all_indexes_rec.def_pct_free;
    io_index_rec.physical_attr_rec.ini_trans := in_all_indexes_rec.def_ini_trans;
    io_index_rec.physical_attr_rec.max_trans := in_all_indexes_rec.def_max_trans;
    io_index_rec.physical_attr_rec.storage.initial_extent := in_all_indexes_rec.def_initial_extent;
    io_index_rec.physical_attr_rec.storage.next_extent := in_all_indexes_rec.def_next_extent;
    io_index_rec.physical_attr_rec.storage.min_extents := in_all_indexes_rec.def_min_extents;
    io_index_rec.physical_attr_rec.storage.max_extents := in_all_indexes_rec.def_max_extents;
    io_index_rec.physical_attr_rec.storage.pct_increase := in_all_indexes_rec.def_pct_increase;
    io_index_rec.physical_attr_rec.storage.freelists := in_all_indexes_rec.def_freelists;
    io_index_rec.physical_attr_rec.storage.freelist_groups := in_all_indexes_rec.def_freelist_groups;
    io_index_rec.physical_attr_rec.storage.buffer_pool := in_all_indexes_rec.def_buffer_pool;
    io_index_rec.logging                := in_all_indexes_rec.def_logging;
    io_index_rec.partitioning_type      := in_all_indexes_rec.partitioning_type;
    io_index_rec.subpartitioning_type   := in_all_indexes_rec.subpartitioning_type;
    io_index_rec.locality               := in_all_indexes_rec.locality;
    io_index_rec.alignment              := in_all_indexes_rec.alignment;
    $IF dbms_db_version.version >= 11 AND dbms_db_version.release >=1 $THEN
    io_index_rec.interval               := in_all_indexes_rec.interval;
    $END
  END assign_part_index_rec;

  PROCEDURE read_join_index_columns(
    io_index_rec  IN OUT NOCOPY gt_index_rec
  )
  AS
  BEGIN
    ...
    FOR i IN 1..io_index_rec.column_arr.COUNT LOOP
      io_index_rec.join_column_indx_arr('"'||io_index_rec.column_table_owner_arr(i)||'"."'||
                                             io_index_rec.column_table_arr(i)||'"."'||
                                             io_index_rec.column_arr(i)||'"') := i;
    END LOOP;
  END read_join_index_columns;
  ...
  AS
    ...
    l_join_ind_columns_arr     gt_all_join_ind_columns_arr;
    l_col_name_arr             arrays.gt_xlstr_arr;
    l_col_position_arr         arrays.gt_str_arr;
    l_descend_arr              arrays.gt_str_arr;
    l_indx                     PLS_INTEGER;
  ...
  BEGIN
    ...
    IF out_index_rec.partitioned = 'YES' THEN
      SELECT column_name
        BULK COLLECT
        INTO out_index_rec.part_key_column_arr
        FROM all_part_key_columns
       WHERE owner = in_owner
         AND name = in_index_name
         AND object_type = 'INDEX'
       ORDER BY column_position;

      IF out_index_rec.subpartitioning_type <> 'NONE' THEN
        SELECT column_name
          BULK COLLECT
          INTO out_index_rec.subpart_key_column_arr
          FROM all_subpart_key_columns
         WHERE owner = in_owner
           AND name = in_index_name
           AND object_type = 'INDEX'
    ...
    -- Read index columns
    IF out_index_rec.index_type IN ('FUNCTION-BASED NORMAL','FUNCTION-BASED NORMAL/REV', 'FUNCTION-BASED BITMAP')
    THEN
      SELECT column_expression
        BULK COLLECT
        INTO out_index_rec.column_arr
        FROM all_ind_expressions
       WHERE index_owner = out_index_rec.owner
         AND index_name = out_index_rec.index_name
       ORDER BY column_position;
    ELSIF out_index_rec.index_type = 'BITMAP' AND out_index_rec.join_index =  'YES' THEN
      SELECT table_owner, table_name, column_name
        BULK COLLECT
        INTO out_index_rec.column_table_owner_arr, out_index_rec.column_table_arr, out_index_rec.column_arr
        FROM all_ind_columns
       WHERE index_owner = out_index_rec.owner
         AND index_name = out_index_rec.index_name
       ORDER BY column_position;
      FOR i IN 1..out_index_rec.column_arr.COUNT LOOP
        out_index_rec.join_column_indx_arr('"'||out_index_rec.column_table_owner_arr(i)||'"."'||
                                                out_index_rec.column_table_arr(i)||'"."'||
                                                out_index_rec.column_arr(i)||'"') := i;
      END LOOP;
      read_join_index_columns(out_index_rec);
    ELSE
      SELECT column_name, descend
        BULK COLLECT
        INTO out_index_rec.column_arr, out_index_rec.sort_order_arr
        FROM all_ind_columns
       WHERE index_owner = out_index_rec.owner
         AND index_name = out_index_rec.index_name
       ORDER BY column_position;
  ...
  AS
    ...
    l_part_indexes_rec         gt_all_part_indexes_rec;
    l_join_ind_columns_arr     gt_all_join_ind_columns_arr;
    l_index_owner_arr          arrays.gt_str_arr;
    l_index_name_arr           arrays.gt_str_arr;
    l_col_name_arr             arrays.gt_xlstr_arr;
    l_col_position_arr         arrays.gt_str_arr;
    l_descend_arr              arrays.gt_str_arr;
    l_indx                     PLS_INTEGER;
  ...
  BEGIN
    ...
    FOR i IN 1..l_all_indexes_arr.COUNT LOOP
      ...
      -- Read attribures for partitioned indexes
      IF io_table_rec.index_arr(i).partitioned = 'YES' THEN
        BEGIN
          SELECT *
            INTO l_part_indexes_rec
            FROM all_part_indexes
           WHERE owner = io_table_rec.index_arr(i).owner
             AND index_name = io_table_rec.index_arr(i).index_name;
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
            RETURN;
        END;

        assign_part_index_rec(
          io_index_rec       => io_table_rec.index_arr(i),
          in_all_indexes_rec => l_part_indexes_rec
        );

        SELECT column_name
          BULK COLLECT
          INTO io_table_rec.index_arr(i).part_key_column_arr
          FROM all_part_key_columns
         WHERE owner = io_table_rec.index_arr(i).owner
           AND name = io_table_rec.index_arr(i).index_name
           AND object_type = 'INDEX'
         ORDER BY column_position;

        IF io_table_rec.index_arr(i).subpartitioning_type <> 'NONE' THEN
          SELECT column_name
            BULK COLLECT
            INTO io_table_rec.index_arr(i).subpart_key_column_arr
            FROM all_subpart_key_columns
           WHERE owner = io_table_rec.index_arr(i).owner
             AND name = io_table_rec.index_arr(i).index_name
             AND object_type = 'INDEX'
    ...
    IF l_all_indexes_arr.COUNT > 0 THEN
     SELECT index_owner, index_name, column_name, column_position, descend
        BULK COLLECT
        INTO l_index_owner_arr, l_index_name_arr, l_col_name_arr, l_col_position_arr, l_descend_arr
        FROM all_ind_columns
       WHERE table_owner = io_table_rec.owner
         AND table_name = io_table_rec.table_name;

      FOR i IN 1..l_col_name_arr.COUNT LOOP
        l_index_full_name := '"'||l_index_owner_arr(i)||'"."'||l_index_name_arr(i)||'"';
        IF io_table_rec.index_indx_arr.EXISTS(l_index_full_name) THEN
          l_indx := io_table_rec.index_indx_arr(l_index_full_name);
          io_table_rec.index_arr(l_indx).column_arr(l_col_position_arr(i)) := l_col_name_arr(i);
          io_table_rec.index_arr(l_indx).sort_order_arr(l_col_position_arr(i)) := l_descend_arr(i);
          io_table_rec.index_arr(l_indx).column_id_arr(l_col_position_arr(i)) := get_column_indx(io_table_rec, l_col_name_arr(i));
        END IF;
      END LOOP;

      SELECT index_owner, index_name, column_expression, column_position
        BULK COLLECT
        INTO l_index_owner_arr, l_index_name_arr, l_col_name_arr, l_col_position_arr
        FROM all_ind_expressions
       WHERE table_owner = io_table_rec.owner
         AND table_name = io_table_rec.table_name;
      ...
      FOR i IN 1..io_table_rec.index_arr.COUNT LOOP
        IF io_table_rec.index_arr(i).index_type = 'BITMAP' AND io_table_rec.index_arr(i).join_index = 'YES' THEN
          SELECT table_owner, table_name, column_name
            BULK COLLECT
            INTO io_table_rec.index_arr(i).column_table_owner_arr, io_table_rec.index_arr(i).column_table_arr, io_table_rec.index_arr(i).column_arr
            FROM all_ind_columns
           WHERE index_owner = io_table_rec.index_arr(i).owner
             AND index_name = io_table_rec.index_arr(i).index_name
           ORDER BY column_position;

          read_join_index_columns(io_table_rec.index_arr(i));
  ...
  BEGIN
    SELECT index_owner, index_name
      BULK COLLECT
      INTO l_index_owner_arr, l_index_name_arr
      FROM all_join_ind_columns
     WHERE outer_table_owner = io_table_rec.owner
       AND outer_table_name = io_table_rec.table_name;
    FOR i IN 1..l_index_name_arr.COUNT LOOP
      l_index_full_name := '"'||l_index_owner_arr(i)||'"."'||l_index_name_arr(i)||'"';
      io_table_rec.join_index_indx_arr(l_index_full_name) := i;
      read_index(
        in_owner      => l_index_owner_arr(i),
        in_index_name => l_index_name_arr(i),
        out_index_rec => io_table_rec.join_index_arr(i)
      );
    END LOOP;
  END read_join_indexes;
  ...
  BEGIN
    ...
    io_constraint_rec.index_owner       := in_all_constraints_rec.index_owner;  -- Oracle bug in cort_all_constraints
      ...
      IF io_table_rec.constraint_arr(j).constraint_type IN ('P','U') THEN
        SELECT r.*
         ...
         WHERE r.r_owner = io_table_rec.constraint_arr(j).owner
           ...
           AND r.table_name <> io_table_rec.table_name -- self referenced FK
        ;
  ...
  AS
    ...
    TYPE t_part_lob_arr IS TABLE OF cort_all_part_lobs%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  BEGIN
    IF io_table_rec.partitioned = 'YES' THEN
      ...
      FOR i IN 1..l_part_lob_arr.COUNT LOOP
        ...
        io_table_rec.lob_arr(l_column_indx).storage.initial_extent  := l_part_lob_arr(i).def_initial_extent;
        io_table_rec.lob_arr(l_column_indx).storage.next_extent     := l_part_lob_arr(i).def_next_extent;
        io_table_rec.lob_arr(l_column_indx).storage.min_extents     := l_part_lob_arr(i).def_min_extents;
        io_table_rec.lob_arr(l_column_indx).storage.max_extents     := l_part_lob_arr(i).def_max_extents;
        io_table_rec.lob_arr(l_column_indx).storage.max_size        := l_part_lob_arr(i).def_max_size;
        io_table_rec.lob_arr(l_column_indx).storage.pct_increase    := l_part_lob_arr(i).def_pct_increase;
        ...
        io_table_rec.lob_arr(l_column_indx).storage.buffer_pool     := l_part_lob_arr(i).def_buffer_pool;
  ...
  AS
    ...
    TYPE gt_lob_partitions_arr IS TABLE OF cort_all_lob_partitions%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  BEGIN
    ...
    FOR i IN 1..l_tab_partitions_arr.COUNT LOOP
      ...
      io_table_rec.partition_arr(i).physical_attr_rec.storage.buffer_pool := l_tab_partitions_arr(i).buffer_pool;
    ...
    FOR i IN 1..l_lob_partitions_arr.COUNT LOOP
      ...
      l_lob_rec.chunk              := l_lob_partitions_arr(i).chunk;
      ...
      l_lob_rec.storage.buffer_pool     := l_lob_partitions_arr(i).buffer_pool;
    ...
    THEN
      ...
      FOR i IN 1..l_ind_partitions_arr.COUNT LOOP
        ...
        io_table_rec.partition_arr(i).physical_attr_rec.storage.buffer_pool := l_ind_partitions_arr(i).buffer_pool;
      ...
      FOR i IN 1..l_tab_partitions_arr.COUNT LOOP
        ...
        io_table_rec.partition_arr(i).overflow_physical_attr_rec.storage.buffer_pool := l_tab_partitions_arr(i).buffer_pool;
  ...
  BEGIN
    ...
    FOR i IN 1..l_tab_subpartitions_arr.COUNT LOOP
      ...
      io_table_rec.subpartition_arr(i).physical_attr_rec.storage.buffer_pool := l_tab_subpartitions_arr(i).buffer_pool;
    ...
    FOR i IN 1..l_lob_subpartitions_arr.COUNT LOOP
      ...
      l_lob_rec.storage.buffer_pool     := l_lob_subpartitions_arr(i).buffer_pool;
  ...
  PROCEDURE read_table_privileges(
    io_table_rec IN OUT NOCOPY gt_table_rec
  )
  AS
  ...
  BEGIN
    SELECT *
     ...
     WHERE table_name = io_table_rec.table_name
       AND table_schema = io_table_rec.owner;

    FOR i IN 1..l_all_tab_privs_arr.COUNT LOOP
      io_table_rec.privilege_arr(i).grantor      := l_all_tab_privs_arr(i).grantor;
      io_table_rec.privilege_arr(i).grantee      := l_all_tab_privs_arr(i).grantee;
      io_table_rec.privilege_arr(i).table_schema := l_all_tab_privs_arr(i).table_schema;
      io_table_rec.privilege_arr(i).table_name   := l_all_tab_privs_arr(i).table_name;
      io_table_rec.privilege_arr(i).privilege    := l_all_tab_privs_arr(i).privilege;
      io_table_rec.privilege_arr(i).grantable    := l_all_tab_privs_arr(i).grantable;
      io_table_rec.privilege_arr(i).hierarchy    := l_all_tab_privs_arr(i).hierarchy;
    END LOOP;
    ...
    FOR i IN 1..l_all_col_privs_arr.COUNT LOOP
      l_indx := io_table_rec.privilege_arr.COUNT + i;
      io_table_rec.privilege_arr(l_indx).grantor      := l_all_col_privs_arr(i).grantor;
      io_table_rec.privilege_arr(l_indx).grantee      := l_all_col_privs_arr(i).grantee;
      io_table_rec.privilege_arr(l_indx).table_schema := l_all_col_privs_arr(i).table_schema;
      io_table_rec.privilege_arr(l_indx).table_name   := l_all_col_privs_arr(i).table_name;
      io_table_rec.privilege_arr(l_indx).column_name  := l_all_col_privs_arr(i).column_name;
      io_table_rec.privilege_arr(l_indx).privilege    := l_all_col_privs_arr(i).privilege;
      io_table_rec.privilege_arr(l_indx).grantable    := l_all_col_privs_arr(i).grantable;
    END LOOP;
  END read_table_privileges;
...
--      io_table_rec.policy_indx_arr(l_full_name) := i;
  ...
  BEGIN
    ...
    FOR i IN 1..io_constraint_arr.COUNT LOOP
      IF io_constraint_arr(i).constraint_type IN ('C', 'F') AND
  ...
  BEGIN
    ...
    THEN
      l_sql := 'INSERT /*+ APPEND '||l_parallel||' */ INTO "'||in_target_table_rec.rename_rec.current_name||'" ('||l_columns_list||') '||
  ...
  AS
    ...
    l_rlbk_stmt_arr    arrays.gt_clob_arr;
  BEGIN
    -- following changes do nbot have rollback plan because applied to temp table
    BEGIN
      ...
      apply_changes_no_rollback(
        in_stmt_arr => l_frwd_stmt_arr,
        in_test     => g_params.test
      );
...
/*
    ...
    -- drop old rollback table if it exists (executed immediately)
    exec_drop_table(
      in_table_name => io_source_table_rec.rename_rec.cort_name,
      in_owner      => io_source_table_rec.owner
    );
*/
    -- following changes are added to the queue 
    IF in_recreate_mode = cort_comp_pkg.gc_result_exchange THEN
    ...
    END IF;
    IF g_params.keep_policies THEN
      cort_comp_pkg.copy_policies(
        in_source_table_rec      => io_source_table_rec,
        in_target_table_rec      => io_target_table_rec,
        io_frwd_alter_stmt_arr   => l_frwd_stmt_arr,
    ...
    -- rename existing (old) table and it's constraints and log groups to cort_name
    ...
    -- rename new table name and it's constraints and log groups to new name
    rename_table_cascade(
  ...
  BEGIN
    ...
    FOR i IN 1..io_target_table_rec.column_arr.COUNT LOOP
      IF io_target_table_rec.column_arr(i).cort_value_indx IS NOT NULL --AND
         --NOT io_target_table_rec.column_arr(i).virtual_col_dependency 
      THEN
        l_cort_value := TRIM(io_target_table_rec.column_arr(i).cort_values(io_target_table_rec.column_arr(i).cort_value_indx).expression);
        ...
        IF l_pos = LENGTH(l_cort_value) THEN
          ...
          IF io_source_table_rec.column_indx_arr.EXISTS(l_col_name) THEN
            ...
            io_source_table_rec.column_arr(l_pos).new_column_name := io_target_table_rec.column_arr(i).column_name;
  ...
  AS
    ...
    l_check_stmt_arr       arrays.gt_xlstr_arr;
  BEGIN
    ...
    FOR i IN 1..io_source_table_rec.column_arr.COUNT LOOP
      IF io_source_table_rec.column_arr(i).cort_value_indx IS NOT NULL AND 
         io_source_table_rec.column_arr(i).cort_values(io_source_table_rec.column_arr(i).cort_value_indx).force_value
      THEN
    ...
    IF l_result < l_stop_vaue THEN
      ...
      mark_notnull_constraints(
        io_constraint_arr => io_source_table_rec.constraint_arr,
        io_column_arr     => io_source_table_rec.column_arr
      );
      --
      mark_notnull_constraints(
        io_constraint_arr => io_target_table_rec.constraint_arr,
        io_column_arr     => io_target_table_rec.column_arr
      );
      -- compare columns
      l_comp_result := cort_comp_pkg.comp_table_columns(
                         ...
                         io_frwd_alter_stmt_arr => io_frwd_alter_stmt_arr,
                         io_rlbk_alter_stmt_arr => io_rlbk_alter_stmt_arr,
                         io_check_stmt_arr      => l_check_stmt_arr
                       );

      IF l_comp_result = cort_comp_pkg.gc_result_alter THEN
        l_comp_result := run_checks(
                           in_source_table_rec  => io_source_table_rec,
                           in_check_stmt_arr    => l_check_stmt_arr
                         );
      END IF;
    ...
    debug('Compare result - '||l_comp_result);
    RETURN l_result;
  END comp_tables;

  -- Public : recreate table
  ...
  AS
    ...
    l_overall_timer        PLS_INTEGER;
    l_timer                PLS_INTEGER;
    l_xml                  XMLTYPE;
  BEGIN
    set_context('CREATE_OR_REPLACE_TABLE',in_table_name);
    g_ddl_arr.DELETE;

    debug('---------------------------');
    
    start_timer(l_overall_timer);
    ...
    cort_log_pkg.log(
      in_text      => 'SESSION_PARAMS',
      in_params    => l_xml.getClobVal()
    );
    ...
    cort_log_pkg.set_logging(g_params.log);
    
    cort_log_pkg.log(
      in_text      => 'INITIAL_PARSE_PARAMS',
      in_params    => l_xml.getClobVal()
    );
    ...
    IF l_old_table_rec.table_name IS NOT NULL THEN
      ...
      IF cort_aux_pkg.get_last_change_sql(
           in_object_owner => in_owner,
           in_object_type  => 'TABLE',
           in_object_name  => in_table_name
         ) = in_sql
      THEN
        debug('No changes since last operation applied');
        stop_timer(l_overall_timer, 'Overall');
      ...
      read_table_columns(l_old_table_rec);
      -- read all indexes for existing table
      read_table_indexes(l_old_table_rec);
      -- read all constraints for existing table
      read_table_constraints(l_old_table_rec);
      -- read all log groups for existing table
      read_table_log_groups(l_old_table_rec);
      -- read all lobs for existing table
      read_table_lobs(l_old_table_rec);
      -- read all XML columns for existing table
      read_table_xml_cols(l_old_table_rec);
      -- read all varray columns for existing table
      read_table_varrays(l_old_table_rec);
      -- read all reference constraints referencing from other tables to existed table
      read_table_references(l_old_table_rec);
      -- read all join indexes on other tables joined with existing table
      read_join_indexes(l_old_table_rec);
      -- find all columns included into join indexes
      read_table_triggers(l_old_table_rec);
      -- read partitions
      read_partitions(l_old_table_rec);
      -- read subpartitions
      read_subpartitions(l_old_table_rec);
      ...
      BEGIN
        ...
        -- read columns of new table
        read_table_columns(l_new_table_rec);
        -- read all indexes for new table
        read_table_indexes(l_new_table_rec);
        -- read all constraints for new table
        read_table_constraints(l_new_table_rec);
        -- read all log groups for new table
        read_table_log_groups(l_new_table_rec);
        -- read all lobs for new table
        read_table_lobs(l_new_table_rec);
        -- read all XML columns for new table
        read_table_xml_cols(l_new_table_rec);
        -- read all varray columns for new table
        read_table_varrays(l_new_table_rec);
        -- read partitions
        read_partitions(l_new_table_rec);
        -- read subpartitions
        read_subpartitions(l_new_table_rec);
        
        -- assign column cort_values
        cort_parse_pkg.parse_create_table_sql(
          in_sql        => in_sql,
        ...
        IF l_result = cort_comp_pkg.gc_result_recreate AND
           l_old_table_rec.partitioned = 'YES' AND
           l_new_table_rec.partitioned = 'YES' AND
           g_params.keep_partitions AND
           cort_comp_pkg.comp_partitioning(
               in_source_table_rec => l_old_table_rec,
               in_target_table_rec => l_new_table_rec
             ) = 0
        THEN
          l_new_sql := in_sql;
          IF g_params.keep_subpartitions AND
             cort_comp_pkg.comp_subpartitioning(
               in_source_table_rec => l_old_table_rec,
               in_target_table_rec => l_new_table_rec
             ) = 0
          THEN 
          ...
          cort_parse_pkg.replace_partitions_sql(
            io_sql           => l_new_sql,
            in_partition_sql => cort_comp_pkg.get_partitions_sql(
          ...
          cort_log_pkg.log(
            in_text      => 'MODIFIED SQL WITH REPLACED PARTITIONS',
            in_params    => l_new_sql
          );
          -- drop temp table
          exec_drop_table(
          ...
          -- re-create table with partitions from original table
          execute_immediate(l_new_sql, FALSE);
        END IF;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          apply_changes_no_rollback(
            in_stmt_arr => l_rlbk_rename_stmt_arr,
            in_echo     => NULL
          );
          RAISE;
      END;
      ...
      WHEN l_result IN (cort_comp_pkg.gc_result_alter, cort_comp_pkg.gc_result_alter_move) THEN
        ...
        IF NOT g_params.test THEN
          cort_aux_pkg.register_change(
            ...
            in_object_name   => in_table_name,
            in_sql           => in_sql,
            in_rollback_name => in_table_name,
      ...
      WHEN l_result IN (cort_comp_pkg.gc_result_exchange, cort_comp_pkg.gc_result_recreate) THEN
        start_timer(l_timer);
        ...
        IF NOT g_params.test THEN
          cort_aux_pkg.register_change(
            ...
            in_object_name   => in_table_name,
            in_sql           => in_sql,
            in_rollback_name => l_old_table_rec.rename_rec.cort_name,
            in_frwd_stmt_arr => l_frwd_stmt_arr, 
            in_rlbk_stmt_arr => l_rlbk_stmt_arr
          );
          COMMIT;
    ...
    set_context('CREATE_OR_REPLACE_TABLE',NULL);
    stop_timer(l_overall_timer, 'Overall');
  END create_or_replace_table;

  -- Public
  PROCEDURE drop_or_rollback_table(
    in_table_name     IN VARCHAR2,
    in_owner          IN VARCHAR2,
    in_sql            IN CLOB,
    in_params_rec     IN cort_params_pkg.gt_params_rec 
  ...
  AS
    ...
    l_xml                  XMLTYPE;
  BEGIN
    set_context('DROP_OR_ROLLBACK_TABLE',in_table_name);
    g_ddl_arr.DELETE;
  ...
  END drop_or_rollback_table;

  -- Rollback the latest change for given object
  PROCEDURE rollback_change(
    in_object_type    IN VARCHAR2,
    in_object_name    IN VARCHAR2,
    in_object_owner   IN VARCHAR2,
    in_params_rec     IN cort_params_pkg.gt_params_rec
  ...
  BEGIN
    ...
    cort_log_pkg.log(
      in_text      => 'ROLLBACK_PARAMS',
      in_params    => l_xml.getClobVal()
    );
    
    
    l_id := cort_aux_pkg.get_last_change_id(
              in_object_owner => in_object_owner,
              in_object_type  => in_object_type,
              in_object_name  => in_object_name
            );
    IF l_id IS NOT NULL THEN
      cort_aux_pkg.get_change_rollback_ddl(l_id, l_rlbk_stmt_arr);

      apply_changes_no_rollback(
        in_stmt_arr    => l_rlbk_stmt_arr,
        in_raise_error => TRUE,
        in_test        => g_params.test,
        in_echo        => TRUE
      );
      ...
      IF NOT g_params.test THEN
        cort_aux_pkg.unregister_last_change(
          in_object_owner => in_object_owner,
          in_object_type  => in_object_type,
          in_object_name  => in_object_name
        );
