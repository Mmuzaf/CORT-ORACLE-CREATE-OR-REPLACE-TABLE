...
*/
  ...
  gc_next                    CONSTANT VARCHAR2(30) := 'NEXT';
...
--  gc_create_index            CONSTANT VARCHAR2(30) := '<create_index>';
  ...
  BEGIN
    ...
    FOR i IN 1..in_source_column_arr.COUNT LOOP
      IF in_source_column_arr.EXISTS(i) AND  
        ...
        THEN
          IF in_check_type AND l_target_column_rec.virtual_column = 'NO' THEN
            -- for segment columns check that column name, data type and position are the same
            IF comp_value(NVL(l_source_column_rec.new_column_name,l_source_column_rec.column_name), l_target_column_rec.column_name) = 0 AND 
               comp_data_type(l_source_column_rec, l_target_column_rec) = 0 AND 
               l_source_column_rec.segment_column_id = l_target_column_rec.segment_column_id
            THEN
              NULL;
            ELSE
              RETURN 1;
            END IF;
          ELSIF in_check_type AND l_target_column_rec.virtual_column = 'YES' THEN
    ...
    THEN
      IF l_source_constraint_rec.r_table_name = l_target_constraint_rec.r_table_name AND
         ...
         comp_column_arr( 
           ...
           in_source_column_arr => l_source_constraint_rec.column_arr, 
           in_target_column_arr => l_target_constraint_rec.column_arr
         ) = 0
      THEN
        RETURN 0;
      ELSE
  ...
  BEGIN
    l_clause_indx_arr(gc_add_constraint) := get_clause(gc_add_constraint, in_constraint_rec.rename_rec.object_name, '"', '"');
    ...
    CASE 
      ...
      WHEN in_constraint_rec.constraint_type IN ('P','U') THEN
        ...
        IF in_index_rec.index_name IS NOT NULL THEN
          l_clause_indx_arr(gc_constraint_index) := get_clause(gc_constraint_index, in_index_rec.rename_rec.object_name, '"', '"');
        END IF;  
  ...
  BEGIN
    ...
    ELSE  
      l_keep_index := NULL;
    END IF;  
    l_rlbk_clause := get_clause(gc_drop_constraint, in_constraint_rec.rename_rec.object_name, '"', '"'||l_keep_index);
  ...
  BEGIN
    ...
    ELSE  
      l_keep_index := NULL;
    END IF;  
    l_frwd_clause := get_clause(gc_drop_constraint, in_constraint_rec.constraint_name, '"', '"'||l_keep_index);
  ...
  AS
    l_clause  CLOB;
  BEGIN
    l_clause := get_clause(gc_next, in_storage_rec.next_extent)||
                get_clause(gc_minextents, in_storage_rec.min_extents)||
                get_clause(gc_maxextents, in_storage_rec.max_extents)||
                get_clause(gc_pctincrease, in_storage_rec.pct_increase)||
                get_clause(gc_freelists, in_storage_rec.freelists)||
                get_clause(gc_freelist_groups, in_storage_rec.freelist_groups)||
                get_clause(gc_buffer_pool, in_storage_rec.buffer_pool);
    RETURN get_clause(gc_storage, l_clause, '(', ')');
  ...
  BEGIN
    l_clause := get_clause(gc_next, in_storage_rec.next_extent)||
                get_clause(gc_minextents, in_storage_rec.min_extents)||
                get_clause(gc_maxextents, in_storage_rec.max_extents)||
                get_clause(gc_maxsize, in_storage_rec.max_size)||
                get_clause(gc_pctincrease, in_storage_rec.pct_increase)||
                get_clause(gc_freelists, in_storage_rec.freelists)||
                get_clause(gc_freelist_groups, in_storage_rec.freelist_groups)||
                get_clause(gc_buffer_pool, in_storage_rec.buffer_pool);
  ...
  AS
    ...
    l_rlbk_clause CLOB;
  BEGIN
    IF comp_value(in_source_storage_rec.next_extent, in_target_storage_rec.next_extent) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_next, in_target_storage_rec.next_extent);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_next, in_source_storage_rec.next_extent);
    END IF;         
      IF comp_value(in_source_storage_rec.min_extents, in_target_storage_rec.min_extents) = 1 THEN
        l_frwd_clause := l_frwd_clause || get_clause(gc_minextents, in_target_storage_rec.min_extents);
        l_rlbk_clause := l_rlbk_clause || get_clause(gc_minextents, in_source_storage_rec.min_extents);
      END IF;         
      IF comp_value(in_source_storage_rec.max_extents, in_target_storage_rec.max_extents) = 1 THEN
        l_frwd_clause := l_frwd_clause || get_clause(gc_maxextents, in_target_storage_rec.max_extents);
        l_rlbk_clause := l_rlbk_clause || get_clause(gc_maxextents, in_source_storage_rec.max_extents);
      END IF;
    IF comp_value(in_source_storage_rec.pct_increase, in_target_storage_rec.pct_increase) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_pctincrease, in_target_storage_rec.pct_increase);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_pctincrease, in_source_storage_rec.pct_increase);
    END IF;        
    IF comp_value(in_source_storage_rec.freelists, in_target_storage_rec.freelists) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_freelists, in_target_storage_rec.freelists);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_freelists, in_source_storage_rec.freelists);
    END IF;           
    IF comp_value(in_source_storage_rec.freelist_groups, in_target_storage_rec.freelist_groups) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_freelist_groups, in_target_storage_rec.freelist_groups);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_freelist_groups, in_source_storage_rec.freelist_groups);
    END IF;     
    IF comp_value(in_source_storage_rec.buffer_pool, in_target_storage_rec.buffer_pool) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_buffer_pool, in_target_storage_rec.buffer_pool);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_buffer_pool, in_source_storage_rec.buffer_pool);
    END IF;   
    IF comp_value(in_source_storage_rec.max_size, in_target_storage_rec.max_size) = 1 THEN
      l_frwd_clause := l_frwd_clause || get_clause(gc_maxsize, in_target_storage_rec.max_size);
      l_rlbk_clause := l_rlbk_clause || get_clause(gc_maxsize, in_source_storage_rec.max_size);
    END IF;   
    out_frwd_clause := l_frwd_clause;
    out_rlbk_clause := l_rlbk_clause;
    IF l_frwd_clause IS NOT NULL THEN
      RETURN 1;
    ELSE
      RETURN 0;  
    END IF;  
  ...
  -- Compares physicale attributes. Returns 0 if they are identical, otherwise returns 1
  ...
  BEGIN
    ...
    IF comp_value(in_source_table_rec.iot_type, in_target_table_rec.iot_type) = 1 THEN
      ...
      RETURN gc_result_recreate;
    END IF; 
    IF comp_column_arr(in_source_table_rec, in_target_table_rec, in_source_table_rec.iot_pk_column_arr, in_target_table_rec.iot_pk_column_arr) = 1 THEN
    ...
    IF comp_value(in_source_table_rec.overflow_table_name, in_target_table_rec.overflow_table_name) = 1 THEN
      IF cort_exec_pkg.g_params.rollback OR 
...
--    END IF; 
    ...
    IF comp_value(in_source_table_rec.subpartitioning_type, in_target_table_rec.subpartitioning_type) = 1 THEN
      ...
      RETURN gc_result_recreate;
    END IF; 
    IF comp_column_arr(in_source_table_rec, in_target_table_rec, in_source_table_rec.part_key_column_arr, in_target_table_rec.part_key_column_arr) = 1 THEN
      debug('Compare part key columns - 1');
      RETURN gc_result_recreate;
    END IF; 
    IF comp_column_arr(in_source_table_rec, in_target_table_rec, in_source_table_rec.subpart_key_column_arr, in_target_table_rec.subpart_key_column_arr) = 1 THEN 
    ...
    IF comp_value(in_source_table_rec.dependencies, in_target_table_rec.dependencies) = 1 THEN
      ...
      RETURN gc_result_recreate;
    END IF; 
    IF comp_value(in_source_table_rec.physical_attr_rec.storage.initial_extent, in_target_table_rec.physical_attr_rec.storage.initial_extent) = 1 THEN
  ...
  BEGIN
    ...
    l_clause_indx_arr(gc_storage) := get_clause(gc_storage, get_storage_clause(in_lob_rec.storage), '(', ')');
  ...
  BEGIN
    ...
    WHEN in_column_rec.data_type = 'XMLTYPE' THEN
      IF in_table_rec.xml_col_arr.EXISTS(l_column_indx) THEN
        ...
        END IF;
        l_clause_indx_arr(gc_store_as) := get_clause(gc_xml_storage_type, l_xml_rec.storage_type, get_clause_by_name(l_clause_indx_arr, gc_securefile));
  ...
  BEGIN
    ...
    IF cort_exec_pkg.g_params.rollback THEN
    ...
    ELSE
      ...
      IF comp_value(in_source_column_rec.data_type, in_target_column_rec.data_type) = 1 OR
        ...
        ELSE
          l_result := gc_result_alter;
          -- scalar data type
          -- rollbackable changes of types
          IF (in_source_column_rec.data_type = 'VARCHAR2' AND in_target_column_rec.data_type = 'CHAR' AND 
              in_source_column_rec.data_length >= in_target_column_rec.data_length) OR
             (in_source_column_rec.data_type = 'CHAR' AND in_target_column_rec.data_type = 'VARCHAR2' AND
              in_source_column_rec.data_length <= in_target_column_rec.data_length) OR
             (in_source_column_rec.data_type = 'NVARCHAR2' AND in_target_column_rec.data_type = 'NCHAR' AND
              in_source_column_rec.data_length >= in_target_column_rec.data_length) OR 
          ...
          THEN
            ...
            l_rlbk_clause_indx_arr(gc_data_type) := get_column_type_clause(in_source_column_rec);
          ELSE
            -- non bi-directionaly altered types
            IF NOT cort_exec_pkg.g_params.rollback AND
              -- non rollback changes of types
              (in_source_column_rec.data_type = 'VARCHAR2' AND in_target_column_rec.data_type = 'CHAR' AND
               in_source_column_rec.data_length > in_target_column_rec.data_length) OR 
              (in_source_column_rec.data_type = 'VARCHAR2' AND in_target_column_rec.data_type = 'NVARCHAR2') OR
              (in_source_column_rec.data_type = 'VARCHAR2' AND in_target_column_rec.data_type = 'NCHAR') OR
              (in_source_column_rec.data_type = 'CHAR' AND in_target_column_rec.data_type = 'NCHAR') 
            THEN   
              l_frwd_clause_indx_arr(gc_data_type) := get_column_type_clause(in_target_column_rec);
              l_rlbk_clause_indx_arr(gc_data_type) := get_column_type_clause(in_source_column_rec);
            ELSE
              debug('Column '||in_source_column_rec.column_name||' - changed type (scalar datatype)');
              RETURN gc_result_recreate;
            END IF;  
          END IF;
        END IF;
      ...
      THEN 
        l_result := gc_result_alter;
        -- rollbackable changes of length
        IF in_target_column_rec.data_type IN ('VARCHAR2','NVARCHAR2','RAW','CHAR','NCHAR') 
        THEN
          l_frwd_clause_indx_arr(gc_data_type) := get_column_type_clause(in_target_column_rec);
          l_rlbk_clause_indx_arr(gc_data_type) := get_column_type_clause(in_source_column_rec);
          IF in_source_column_rec.data_length > in_target_column_rec.data_length THEN
            -- add check for possibility to apply alter
            IF in_source_column_rec.data_type = 'RAW' THEN
              l_check_clause := 'CASE WHEN NVL(MAX(UTL_RAW.LENGTH("'||in_source_column_rec.column_name||'")),0) <= '||
                                 in_target_column_rec.data_length||' THEN 1 ELSE 2 END';
            ELSE
              l_check_clause := 'CASE WHEN NVL(MAX(LENGTH("'||in_source_column_rec.column_name||'")),0) <= '||
                                 in_target_column_rec.data_length||' THEN 1 ELSE 2 END';
            END IF;     
            io_check_stmt_arr(io_check_stmt_arr.COUNT+1) := l_check_clause; 
        ...
        THEN
          ...
          l_rlbk_clause_indx_arr(gc_data_type) := get_column_type_clause(in_source_column_rec);
        ELSE
        -- non-rollbackable changes of length
          IF cort_exec_pkg.g_params.rollback AND 
             ((in_target_column_rec.data_type = 'NUMBER' AND 
          ...
          THEN
            ...
            l_rlbk_clause_indx_arr(gc_data_type) := get_column_type_clause(in_source_column_rec);
          ELSE
            debug('Column '||in_source_column_rec.column_name||' - changed data_length/data_scale/data_precision');
            RETURN gc_result_recreate;
          END IF;   
        END IF;
      END IF;
  ...
  BEGIN
    ...
    -- find added and removed segment columns
    ...
    IF l_drop_column_arr.COUNT > 0 THEN
      ...
      WHILE l_indx1 IS NOT NULL LOOP
        ...
        THEN
          debug('There is added column on the same position and with the same data type as dropped column - column get renamed');
    ...
    IF l_drop_column_arr.COUNT > 0 THEN
      ...
      ELSE
        l_segment_result := gc_result_alter;
      END IF;
      -- if all column need to be droppped then recreate
    ...
    WHILE l_indx1 IS NOT NULL AND l_indx2 IS NOT NULL LOOP
      l_comp_result := comp_column(
                         ...
                         io_check_stmt_arr      => io_check_stmt_arr
                       );
    ...
    -- if there are dropped virtual columns in the middle of the list and rollback mode is enabled
    ...
    THEN
      ...
      IF l_indx1 IS NOT NULL AND cort_exec_pkg.g_params.rollback  
      THEN
        -- then need to recreate
        l_virtual_result := gc_result_exchange;
      ELSE
    ...
    WHILE l_indx1 IS NOT NULL LOOP
      ...
      IF (io_source_table_rec.column_arr(l_indx1).partition_key) OR 
         (io_source_table_rec.column_arr(l_indx1).virtual_col_dependency) OR
      ...
      THEN
        ...
        ELSE
          IF NOT io_source_table_rec.is_table_empty THEN
            raise_application_error(-20000, 'New NOT NULL column '||io_target_table_rec.column_arr(l_indx2).column_name||' must have DEFAULT or cort_VALUE value', TRUE);
          END IF;  
        END IF;  
      END IF;
  ...
  BEGIN
    ...
    l_clause_indx_arr(gc_index_columns) := get_clause(gc_index_columns, convert_arr_to_str(in_index_rec.column_arr), '(', ')');
    ...
    -- physical_attributes
    l_clause_indx_arr(gc_physical_attr) := get_physical_attr_clause(in_index_rec.physical_attr_rec);
  ...
  BEGIN
    ...
    IF comp_value(io_source_index_rec.index_type, io_target_index_rec.index_type) = 1 OR
       comp_array(io_source_index_rec.column_id_arr, io_target_index_rec.column_id_arr) = 1 OR
  ...
  BEGIN
    FOR i IN 1..in_source_table_rec.index_arr.COUNT LOOP
      IF in_source_table_rec.index_arr(i).constraint_name IS NULL OR in_copy_pk_uk THEN
        ...
        l_index_full_name := '"'||io_target_table_rec.index_arr(i).owner||'"."'||io_target_table_rec.index_arr(i).index_name||'"';
        io_target_table_rec.index_indx_arr(l_index_full_name) := l_indx;
        create_index(
          in_index_rec             => io_target_table_rec.index_arr(l_indx),
          io_frwd_alter_stmt_arr   => io_frwd_alter_stmt_arr,
          io_rlbk_alter_stmt_arr   => io_rlbk_alter_stmt_arr
        );
      END IF;
  ...
  BEGIN
    ...
    IF comp_value(in_source_constraint_rec.constraint_type, in_target_constraint_rec.constraint_type) = 1 OR
       comp_array(in_source_constraint_rec.column_id_arr, in_target_constraint_rec.column_id_arr) = 1 OR
    ...
    l_frwd_clause_indx_arr(gc_constraint_name) := get_clause(gc_modify_constraint, in_target_constraint_rec.constraint_name, '"', '"');
  ...
  FUNCTION get_columns_hash_str(
    in_column_id_arr    IN arrays.gt_int_arr
  )
  RETURN VARCHAR2
  AS
    l_col_str      VARCHAR2(32767);
    l_col_indx_arr arrays.gt_int_arr;
    l_indx         PLS_INTEGER;
  BEGIN
    -- sort by value
    FOR i IN 1..in_column_id_arr.COUNT LOOP
      l_col_indx_arr(in_column_id_arr(i)) := i;
    END LOOP;
    
    -- get string regardless position of column in array (ordered by ID)
    l_indx := l_col_indx_arr.FIRST;
    l_col_str := l_indx;
    LOOP
      l_indx := l_col_indx_arr.NEXT(l_indx);
      EXIT WHEN l_indx IS NULL;
      l_col_str := l_col_str || ',' || l_indx;
    END LOOP;
    RETURN l_col_str;
  END get_columns_hash_str;
  ...
  BEGIN
    CASE 
    WHEN in_constraint_rec.constraint_type IN ('C', 'F') THEN
      l_result := 'C:'||in_constraint_rec.search_condition;
    WHEN in_constraint_rec.constraint_type IN ('P', 'U') THEN 
      l_result := 'P:'||get_columns_hash_str(in_constraint_rec.column_id_arr);
    WHEN in_constraint_rec.constraint_type IN ('R') THEN 
      l_result := 'R:'||get_columns_hash_str(in_constraint_rec.column_id_arr)||' R "'||
                  in_constraint_rec.r_owner||'"."'||in_constraint_rec.r_table_name||'":';
      IF in_constraint_rec.r_table_name = in_constraint_rec.table_name AND
         in_constraint_rec.r_owner = in_constraint_rec.owner 
      THEN
        l_result := l_result || get_columns_hash_str(in_constraint_rec.r_column_id_arr);
      ELSE
        l_result := l_result || convert_arr_to_str(in_constraint_rec.r_column_arr);
      END IF;  
    END CASE;
    RETURN get_hash_value(l_result);
  ...
  AS
    l_old_column_indx_arr  arrays.gt_int_arr;
    l_new_column_indx_arr  arrays.gt_int_arr;
    l_old_column_arr       arrays.gt_str_arr;
    l_indx                 PLS_INTEGER;
  BEGIN
    -- Check that set of KEY columns has not been changed
    IF comp_value(
         in_val1 => get_columns_hash_str(in_old_pk_constraint_rec.column_id_arr), 
         in_val2 => get_columns_hash_str(in_new_pk_constraint_rec.column_id_arr)
       ) = 0 
    THEN
      ...
      IF comp_array(in_old_pk_constraint_rec.column_arr, in_new_pk_constraint_rec.column_arr) = 1 THEN
        ...
        FOR i IN 1..in_old_pk_constraint_rec.column_id_arr.COUNT LOOP
          l_old_column_indx_arr(in_old_pk_constraint_rec.column_id_arr(i)) := i;
        END LOOP;      
        FOR i IN 1..in_new_pk_constraint_rec.column_id_arr.COUNT LOOP
          l_new_column_indx_arr(in_new_pk_constraint_rec.column_id_arr(i)) := i;
        END LOOP;      
        ...
        FOR i IN 1..io_ref_contraint_arr.COUNT LOOP
          ...
          l_indx := l_old_column_indx_arr.FIRST;
          WHILE l_indx IS NOT NULL LOOP
            io_ref_contraint_arr(i).column_arr(l_new_column_indx_arr(l_indx)) := l_old_column_arr(l_old_column_indx_arr(l_indx));
            l_indx := l_old_column_indx_arr.NEXT(l_indx);
          END LOOP;
    ...
    ELSE
      ...
      IF cort_exec_pkg.g_params.keep_refs AND cort_exec_pkg.g_params.keep_bad_refs THEN
        raise_application_error(-20000, 'Unable to preserve references on changing primary/unique key', TRUE);
      ELSE
    ...
    WHILE l_hash_index IS NOT NULL LOOP
      ...
      IF NOT io_source_table_rec.constraint_arr(l_source_hash_cons_arr(l_hash_index)).drop_flag THEN
        ...
        ELSE
          ...
          THEN
            IF cort_exec_pkg.g_params.keep_refs AND cort_exec_pkg.g_params.keep_bad_refs THEN
              raise_application_error(-20000, 'Unable to preserve references on droping primary/unique key', TRUE);
            END IF;
            drop_references(
              in_table_rec           => io_source_table_rec,
              in_pk_constraint_rec   => l_source_constraint_rec,
  ...
  AS
  BEGIN
    RETURN in_log_group_rec.log_group_type||':'||get_hash_value(convert_arr_to_str(in_log_group_rec.column_arr)||':'||convert_arr_to_str(in_log_group_rec.column_log_arr));
  ...
  AS
  BEGIN
    RETURN get_hash_value(in_partition_rec.high_value);
  ...
  BEGIN
    ...
    -- Overflow
    IF in_partition_rec.overflow_tablespace IS NOT NULL THEN
      l_clause_indx_arr(gc_overflow_tablespace) := get_clause(gc_overflow_tablespace, in_partition_rec.overflow_tablespace, '"', '"'); 
      -- Physical attrs
      l_clause_indx_arr(gc_overflow_physical_attr) := get_physical_attr_clause(in_partition_rec.overflow_physical_attr_rec);
      -- logging
    ...
    -- subpartitions
    --IF in_partition_rec.partition_level = 'PARTITION' THEN
  ...
  BEGIN
    ...
    FOR i IN 1..in_table_rec.constraint_arr.COUNT LOOP
      ...
      IF in_table_rec.constraint_arr(i).constraint_type IN ('P','U') THEN
        ...
        l_columns_list := l_columns_list||CHR(10)||REGEXP_REPLACE(get_add_constraint_clause(l_constraint_rec, l_index_rec), 'ADD CONSTRAINT', '', 1, 1)||',';
      END IF;
    END LOOP;
    ...
    THEN
      IF is_subpartitioning_available(in_table_rec) THEN
        ...
        l_part_rec.physical_attr_rec  := in_table_rec.physical_attr_rec;
        ...
        l_partition_sql := '
        PARTITION BY LIST("'||in_swap_table_rec.part_key_column_arr(1)||'")
          SUBPARTITION BY '||in_table_rec.partitioning_type||'('||convert_arr_to_str(in_table_rec.part_key_column_arr)||')
        ('||get_partitions_sql(l_part_arr, l_subpart_arr)||')';
      ELSE
    ...
    ELSIF in_table_rec.subpartitioning_type <> 'NONE' THEN
      ...
      l_partition_sql := 'PARTITION BY '||in_table_rec.subpartitioning_type||' ('||convert_arr_to_str(in_table_rec.subpart_key_column_arr)||')  
      ('||get_partitions_sql(l_part_arr, l_subpart_arr)||')';
    END IF;
  ...
  BEGIN
   io_target_table_rec.privilege_arr.DELETE;
    ...
    FOR i IN 1..io_target_table_rec.privilege_arr.COUNT LOOP
      l_frwd_clause := get_grant_privilege_clause(io_target_table_rec.privilege_arr(i));
      l_rlbk_clause := get_revoke_privilege_clause(io_target_table_rec.privilege_arr(i));
      io_frwd_alter_stmt_arr(io_frwd_alter_stmt_arr.COUNT+1) := l_frwd_clause;
      io_rlbk_alter_stmt_arr(io_rlbk_alter_stmt_arr.COUNT+1) := l_rlbk_clause;
    END LOOP;  
  ...
  AS
    ...
    l_indx              PLS_INTEGER;
  BEGIN
    l_clause_indx_arr(gc_create_trigger) := get_clause(gc_create_trigger, in_trigger_rec.owner||'"."'||in_trigger_rec.trigger_name, '"','"');
    ...
    IF in_trigger_rec.referenced_trigger_indx IS NOT NULL THEN
      IF in_table_rec.trigger_arr.EXISTS(in_trigger_rec.referenced_trigger_indx) THEN
        ...
        l_clause_indx_arr(gc_follows) := get_clause(gc_follows, in_table_rec.trigger_arr(l_indx).owner||'"."'||in_table_rec.trigger_arr(l_indx).trigger_name, '"', '"');
      END IF;  
    END IF;  
  ...
  AS
    l_clause            CLOB;
  BEGIN
    l_clause := get_clause(gc_drop_trigger, in_trigger_rec.owner||'"."'||in_trigger_rec.trigger_name, '"','"');
  ...
  BEGIN   
    FOR i IN 1..in_partition_arr.COUNT LOOP
      l_sql := l_sql || get_partition_clause(in_partition_arr(i), 'PARTITION') || 
    ...
    FOR i IN 1..in_target_table_rec.column_arr.COUNT LOOP
      ...
      THEN
        ...
        IF in_source_table_rec.column_indx_arr.EXISTS(in_target_table_rec.column_arr(i).column_name) THEN
          ...
          END IF;
        ELSIF l_cort_indx IS NOT NULL THEN
          out_values_list := out_values_list||in_target_table_rec.column_arr(i).cort_values(l_cort_indx).expression||l_column_alias;
        ELSE  
