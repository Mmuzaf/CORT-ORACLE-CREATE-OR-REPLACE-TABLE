...
*/
  ...
  gc_cort_text_prefix        CONSTANT VARCHAR2(1)  := '#';
  gc_cort_rlbk_prefix        CONSTANT VARCHAR2(5)  := 'rlbk#';
  gc_temp_prefix             CONSTANT VARCHAR2(5)  := '~tmp#';
  gc_swap_prefix             CONSTANT VARCHAR2(5)  := '~swp#';
  gc_stat_table_name         CONSTANT VARCHAR2(30) := 'CORT_STAT';
  gc_default_alias           CONSTANT VARCHAR2(1)  := 'a';

  gc_max_end_time            CONSTANT TIMESTAMP := TIMESTAMP'9999-12-31 23:59:59';
  ...
  -- column level params
  gc_value                   CONSTANT VARCHAR2(30) := '=';
  gc_force_value             CONSTANT VARCHAR2(30) := '==';
  ...
  TYPE gt_storage_rec IS RECORD(
    ...
    next_extent                  VARCHAR2(40), -- Size of secondary extents (in bytes); NULL for partitioned tables
    min_extents                  VARCHAR2(40), -- Minimum number of extents allowed in the segment; NULL for partitioned tables
    max_extents                  VARCHAR2(40), -- Maximum number of extents allowed in the segment; NULL for partitioned tables
...
--    encrypt   for tablespaces only (NOT USED)                                           
  ...
  TYPE gt_index_rec IS RECORD(
    ...
    join_inner_owner_arr         arrays.gt_str_arr,   -- Array of owner of join inner table
    join_inner_table_arr         arrays.gt_str_arr,   -- Array of name of join inner table
    join_inner_column_arr        arrays.gt_str_arr,   -- Array of column of join inner table
    join_outer_owner_arr         arrays.gt_str_arr,   -- Array of owner of join inner table
    join_outer_table_arr         arrays.gt_str_arr,   -- Array of name of join inner table
    join_outer_column_arr        arrays.gt_str_arr,   -- Array of column of join inner table
    join_column_indx_arr         arrays.gt_int_indx,  -- Index by owner_name.table_name.column_name
    ...
    column_arr                   arrays.gt_xlstr_arr, -- Column names or expressions ordered by position
    column_id_arr                arrays.gt_int_arr,   -- Column IDs ordered by position
  ...
  TYPE gt_constraint_rec IS RECORD(
    ...
    r_column_id_arr              arrays.gt_int_arr, -- Referencing table key column IDs ordered by position
  ...
  TYPE gt_column_rec IS RECORD(
    ...
    nullable                     VARCHAR2(1),   -- Indicates whether a column allows NULLs. The value is N if there is aNOT NULL constraint on the column or if the column is part of aPRIMARY KEY
    ...
    qualified_col_hash           VARCHAR2(255), -- Hash value based on qualified column name
    ...
    virtual_col_dependency       BOOLEAN,       -- TRUE if this column is used in virtual columns' expression
  ...
  TYPE gt_table_rec      IS RECORD(
    ...
    column_hash_indx_arr         arrays.gt_int_indx, -- index by hash of qualified column name
  ...
  g_params         cort_params_pkg.gt_params_rec := cort_params_pkg.get_default_rec;
