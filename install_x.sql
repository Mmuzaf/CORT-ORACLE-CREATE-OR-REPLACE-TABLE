...
/*
  Description: advanced install script (requires SQLPLus Extensions). To install call: sqlplus /nolog @install_x.sql
  ----------------------------------------------------------------------------------------------------------------------     
  Release | Author(s)         | Comments
  ----------------------------------------------------------------------------------------------------------------------  
  14.01.1 | Rustam Kafarov    | Renamed from install.sql. Added call of cort_schema_x
  ----------------------------------------------------------------------------------------------------------------------  
*/
...
@IF "(SELECT COUNT(*) FROM ALL_USERS WHERE username = UPPER('&cort_user')) > 0" 
@THEN{
  PROMPT User CORT already exists
  PROMPT Do you want to recreate CORT user?
~ @switch recreate_flg "YES|NO"  
}
@ENDIF

@IF "upper('&recreate_flg') = 'YES'" 
@THEN{
  PROMPT Dropping existing user CORT ...
  DROP USER &cort_user CASCADE; 
}
@ENDIF

@IF "upper('&recreate_flg') = 'NO'"
@ELSE{
  PROMPT Creating user...
  CREATE USER &cort_user IDENTIFIED BY "&cort_psw"; 
}
@ENDIF
